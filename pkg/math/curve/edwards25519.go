...
var (
	// edwards25519Cofactor is the scalar 8, the cofactor of the group.
	edwards25519Cofactor edwards25519.Scalar
	// edwards25519CofactorInverse is 8⁻¹ mod l.
	edwards25519CofactorInverse edwards25519.Scalar
)

func init() {
	var cofactorBytes [32]byte
	cofactorBytes[0] = 8
	edwards25519Cofactor.SetCanonicalBytes(cofactorBytes[:])
	edwards25519CofactorInverse.Invert(&edwards25519Cofactor)
}

// IsInPrimeOrderGroup implements curve.Point.
//
// The edwards25519 group has order 8·l: every point P decomposes uniquely as
// P = Pₜ + Pₚ with [8]Pₜ = O and [l]Pₚ = O, and P belongs to the prime-order
// subgroup generated by the base point iff Pₜ = O. Since 8 is invertible
// modulo l, [8]([8⁻¹ mod l]P) = Pₚ, so comparing that value with P itself
// detects any small-subgroup component.
func (p *Edwards25519Point) IsInPrimeOrderGroup() bool {
	if p == nil {
		return false
	}
	var tmp Edwards25519Point
	tmp.value.ScalarMult(&edwards25519CofactorInverse, &p.value)
	tmp.value.ScalarMult(&edwards25519Cofactor, &tmp.value)
	return tmp.value.Equal(&p.value) == 1
}
