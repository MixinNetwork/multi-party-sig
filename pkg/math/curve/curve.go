...
type Point interface {
	...
	// IsInPrimeOrderGroup checks if this point belongs to the prime-order
	// subgroup generated by the base point of this group.
	//
	// On curves with a cofactor greater than 1 (like edwards25519), points can
	// carry small-order components outside this subgroup. Accepting such
	// points from another party enables small-subgroup attacks, in which
	// attacker-chosen torsion pollutes derived key material while passing
	// every protocol check, so points received over the wire must be
	// validated with this method (see RFC 9591, Section 6.1).
	IsInPrimeOrderGroup() bool
